...
pub const DIRECTIONS: &[(isize, isize)] = &[(0, -1), (-1, 0), (1, 0), (0, 1)];
...
impl<T: Default + Clone> Grid<T> {
    ...
    pub fn new_with<V: IntoIterator<Item = T>>(
        width: usize,
        height: usize,
...
impl<T: Default> Grid<T> {
    ...
    pub fn pos_to_idx(&self, c: &Coordinate) -> Option<usize> {
        if !self.pos_is_in_bounds(c) {
            None
        } else {
            Some(self.pos_to_idx_unchecked(c))
        }
    }
    ...
    pub fn iter_cols(&self) -> impl Iterator<Item = impl Iterator<Item = &T>> {
        (0..self.width).map(|col| self.data[col..].iter().step_by(self.width))
    }

    pub fn iter_rows_enumerated(&self) -> impl Iterator<Item = (Coordinate, &T)> {
        self.iter()
            .enumerate()
            .map(|(i, v)| (self.idx_to_pos_unchecked(i), v))
    }
    ...
    pub fn iter_rows_enumerated_mut(&mut self) -> impl Iterator<Item = (Coordinate, &mut T)> {
        ...
        self.iter_mut()
            .enumerate()
            .map(move |(i, v)| (((i % width) as isize, (i / width) as isize), v))
    }

    pub fn iter_coords(&self) -> impl Iterator<Item = Coordinate> {
        self.iter()
            .enumerate()
            .map(|(i, _)| self.idx_to_pos_unchecked(i))
    }
}

impl<T: Default + Clone + PartialEq> Grid<T> {
    pub fn coord(&self, val: T) -> Option<Coordinate> {
        self.iter()
            .position(|x| *x == val)
            .map(|i| self.idx_to_pos_unchecked(i))
    }
}
